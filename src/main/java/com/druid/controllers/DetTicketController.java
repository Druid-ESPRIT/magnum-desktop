...
public class DetTicketController implements Initializable {
  ...
  @FXML private Button Env;

  Connection cnx = DBConnection.getInstance().getConnection();
  @FXML private DatePicker CreationDate;

  @FXML private TextField Subject;
  @FXML private Button Aff;
  @FXML public TextField test;
  @FXML public Label Cat;
  String S;
  int I;
  ...
  private void initialize(ActionEvent event) throws IOException, SQLException {
    ...
    if (option.isPresent() && option.get() == ButtonType.OK) {
      ...
      if ((!"".equals(Description.getText()))
          && (connectedUser.getID() > 0)
          && (!"".equals(Subject.getText()))
        ...
        try {
          ...
          String req =
              "INSERT INTO `ticket`(`ID`, `Subject`, `Description`, `CreationDate`, `ReSolverID`,"
                  + " `USERID`, `STATUS`,`Evaluate`) VALUES ('"
                  + T1.getID()
                  + "','"
                  + T1.getSubject()
                  + "','"
        ...
        try {
          ...
          String req2 =
              "INSERT INTO `ticketkind`(`ID`, `Itype`, `Type`) VALUES ('"
                  + T1.getID()
                  + "','"
                  + I
                  + "','"
