...
public class EventService implements IEventService {
  ...
  public boolean addEvent(Event e) {
    ...
    String request =
        "INSERT INTO event (user_id, name, description, type, location, date, payant, prix,"
            + " status,image,max_participants) VALUES (?,?,?,?,?,?,?,?,'Not Finished',?,2)";
    try {
  ...
  public boolean updateEvent(Event e) {
    String request =
        "UPDATE event SET user_id=?, name=?, description=?, location=?, date=?, payant=?, prix=?"
            + " WHERE id=?";
    try {
      PreparedStatement pst = cnx.prepareStatement(request);
      pst.setInt(1, e.getUser().getID());
  ...
  public boolean userParticipation(Event e, User u) {
    String request = "INSERT INTO user_event (event_id,user_id)" + " VALUES (?,?)";
