...
public class OrderService {
  ...
  public int addOrder(Order or) {
    int risultato = 0;
    String query =
        "INSERT INTO `order`(`offer_id`,`user_id`,`plan`, `total`, `orderdate`,`status`)VALUES ('"
            + or.getofferId()
            + "','"
            + or.getUser_id()
            + "','"
  ...
  public Timestamp getOrderTimeById(int id) {
    ...
    try {
      ...
      ResultSet result = stmt.executeQuery(query);
      while (result.next()) {
        return (result.getTimestamp("orderdate"));
      }
  ...
  public int getOfferPrice(int id) {
    String query =
        "SELECT offer.price from `order` inner join `offer` on order.offer_id = offer.id where"
            + " order.id ='"
            + id
            + "'";
    try {
  ...
  public int getOrderCount(int id) {
    ...
    String query =
        "SELECT id from `order` where offer_id ='"
            + id
            + "' and status = '"
